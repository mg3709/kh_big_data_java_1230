...
public class ParentPoly {
    protected int num;//protected 접근제한자는 동일한 패키지와 상속관게에서 접근을 허용
    public void display(){
...
class ChildPoly extends ParentPoly{
    ...
    public void out(){//해당 메소드는 부모가 가지지 않은 메소드
        System.out.println("부모 protected num 필드 : " + num);
        System.out.println("자식 클래스의 메소드");
    }
    @Override//해당 메소드가 오버라이딩 관계인지를 컴파일러가 컴파일 시 문법체크를 해주도록 설정
    public void display(){
