...
//        list.stream().filter((n->n % 2 != 0)).forEach(e->System.out.print(e + " "));
        ...
        //스트림 변환 : 해당 스트림의 요소들에 주어진 함수의 인수로 전달하여, 그 반환값으로 이루어진 새로운 스트림 생성
//        Stream<String> stream = Stream.of("HTML", "CSS", "jAVA", "JAVASCRIPT");
//        stream.map(e -> e.length()).forEach(System.out::println);
//
//        String[] arr = {"I study JAVA","You study JAVA","I am hungry"};
//        Stream<String> stream3 = Arrays.stream(arr);
//        stream3.flatMap(s->Stream.of(s.split(" "))).forEach(System.out::println);

        //스트림 제한
        //limit() : 해당 스트림의 첫번째 요소부터 전달된 개수만큼을 요소로만 이루어진 새로운 스트림 반환
        //skip() : 해당 스트림의 첫번째 요소부터 전달된 개수 만큼의 요소를 제외하고 출력
//        IntStream stream4 = IntStream.range(0,10);//0 ~ 10미만까지 범위 지정
